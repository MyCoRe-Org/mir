...
public class MIRModsEditorController extends MIREditorController {
    ...
    public void setTitleInfo(List<MIRTitleInfo> titleInfos) {
        if (titleInfos.size() > 0) {
            ...
            IntStream.range(0, titleInfos.size()).forEach(i -> {
                ...
                WebElement langSelectElement = driver.waitAndFindElement(By.xpath(".//select[contains(@name, '" + baseXP + "@xml:lang" + "')]"));
                new Select(langSelectElement).selectByValue(titleInfo.getLang().getValue());

                WebElement selectTypeElement = driver.waitAndFindElement(By.xpath(".//select[contains(@name, '" + baseXP + "@type" + "')]"));
    ...
    public void setTitleAndTranslation(String title, String subTitle, String translatedTitle, String translatedSubTitle, MIRLanguage language) {
        ...
        new Select(driver.waitAndFindElement(By.xpath(".//select[contains(@name, 'mods:titleInfo[2]/@xml:lang')]"))).selectByValue(language.getValue());
    ...
    public void setOpenAIRE(String searchTrigger, String fullName) {
        ...
        driver.waitAndFindElements(By.xpath(".//ul[contains(@class,'typeahead')]/li/a")).stream().filter(e -> e.getText().contains(fullName)).limit(1).forEach(e -> e.click());
    }
    ...
    public void setAbstracts(List<MIRAbstract> abstracts) {
        if (abstracts.size() > 0) {
            ...
            IntStream.range(0, abstracts.size()).forEach(i -> {
                ...
                new Select(driver.waitAndFindElement(By.xpath(".//select[contains(@name, '" + xp + "/@xml:lang')]"))).selectByValue(anAbstract.getLanguage().getValue());
    ...
    public void setTypeOfResource(MIRTypeOfResource typeOfResource) {
        Select resourceSelect = new Select(driver.waitAndFindElement(By.xpath(".//select[contains(@name, 'mods:typeOfResource/@mcr:categId')]")));
    ...
    public void setTypeOfResources(List<MIRTypeOfResource> typeOfResources){
        if (typeOfResources.size() > 0) {
            ...
            IntStream.range(0, typeOfResources.size()).forEach(i -> {
                String xp = i == 0 ? "mods:typeOfResource/@mcr:categId" : "mods:typeOfResource[" + (i + 1) + "]/@mcr:categId";
                MIRTypeOfResource typeOfResource = typeOfResources.get(i);
                new Select(driver.waitAndFindElement(By.xpath(".//select[contains(@name, '" + xp + "')]"))).selectByValue(typeOfResource.getValue());
            });
    ...
    public void setGenres(List<MIRGenre> genres) {
        if (genres.size() > 0) {
            if (genres.size() > 1) {
                IntStream.range(1, genres.size()).forEach((n) -> clickRepeater("mods:genre"));
            }
            ...
            IntStream.range(0, genres.size()).forEach(i -> {
                ...
                new Select(driver.waitAndFindElement(By.xpath(".//select[contains(@name, '" + xp + "')]"))).selectByValue(currentLang.getValue());
            });
    ...
    public void setLanguages(List<MIRLanguage> langs) {
        if (langs.size() > 0) {
            ...
            IntStream.range(0, langs.size()).forEach(i -> {
                String xp = i == 0 ? "mods:language/mods:languageTerm" : "mods:language[" + (i + 1) + "]/mods:languageTerm";
    ...
    public void setInstitution(MIRInstitutes institution) {
        new Select(driver.waitAndFindElement(By.xpath(".//select[contains(@name, 'mods:name') and option/@value='"+institution.getValue()+"']"))).selectByValue(institution.getValue());
    }
    ...
    public void setIdentifier(List<AbstractMap.Entry<MIRIdentifier, String>> typeIdentifierList, int existing, int nonRepeaterExisting) {
        if (typeIdentifierList.size() > 0) {
            if (typeIdentifierList.size() > (existing)) {
                IntStream.range(1, typeIdentifierList.size()).forEach(n ->
                        clickRepeater("mods:identifier"));
            }
            ...
            IntStream.range(nonRepeaterExisting, nonRepeaterExisting+typeIdentifierList.size()).forEach(i -> {
                ...
                new Select(driver.waitAndFindElement(By.xpath(".//select[contains(@name, '" + selectXP + "')]"))).selectByValue(mirIdentifier.toString());
    ...
    public void setTopics(List<String> topics) {
        if (topics.size() > 0) {
            if (topics.size() > 1) {
                IntStream.range(1, topics.size()).forEach((n) -> clickRepeater("mods:subject"));
            }
    ...
    public void setClassifications(List<MIRDNBClassification> classifications) {
        if (classifications.size() > 0) {
            if (classifications.size() > 1) {
                IntStream.range(1, classifications.size()).forEach((n) -> clickRepeater("mods:classification"));
            }
            ...
            IntStream.range(0, classifications.size()).forEach(i -> {
                ...
                new Select(driver.waitAndFindElement(By.xpath(".//select[contains(@name, '" + xp + "')]"))).selectByValue(classification.getValue());
            });
        }
    }
    ...
    private void setByValueXpath(String valueXpath, String value) {
        WebElement webElement = driver.waitAndFindElement(By.xpath(".//input[contains(@data-valuexpath, \"" + valueXpath + "\")]"));
        webElement.clear();
    ...
    public boolean isClassificationValidationMessageVisible() {
        return hasValidationText(MIRTestData.VALIDATION_CLASSIFICATION) && this.hasInputTextError("mods:classification");
    }
