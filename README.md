...
|--------------------------------|---------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| ENABLE_SOLR_CLOUD              | false               | If true the Solr Cloud mode is enabled. (solr cores/collections will be created on install). Sets `MCR.Solr.IndexManager.Collection.*.Class` to `MCRConfigurableSolrCloudCollection` instead of `MCRConfigurableSolrCore`.                                                                        |
| SOLR_URL                       | none                | The HTTP URL to the Solr server. In standalone mode sets `MCR.Solr.IndexManager.Collection.*.SolrUrl`. In cloud mode sets `MCR.Solr.IndexManager.Collection.*.SolrUrls`. Not required if `SOLR_ZK_HOST` is set in cloud mode.                                                                     |
| SOLR_ZK_HOST                   | none                | ZooKeeper URL(s) for SolrCloud (comma-separated). Sets `MCR.Solr.IndexManager.Collection.*.ZkUrl`. Only used when `ENABLE_SOLR_CLOUD=true`. Takes precedence over `SOLR_URL` for the connection when both are set.                                                                                |
| SOLR_ZK_CHROOT                 | none                | Optional ZooKeeper chroot path for SolrCloud. Sets `MCR.Solr.IndexManager.Collection.*.ZkChroot`. Only used when `SOLR_ZK_HOST` is set.                                                                                                                                                          |
| SOLR_CORE                      | mir                 | In standalone mode: the core name (`MCR.Solr.IndexManager.Collection.main.CoreName`). In cloud mode: the collection name (`MCR.Solr.IndexManager.Collection.main.CollectionName`).                                                                                                                |
| SOLR_CLASSIFICATION_CORE       | mir-classifications | In standalone mode: the classification core name (`MCR.Solr.IndexManager.Collection.classification.CoreName`). In cloud mode: the classification collection name (`MCR.Solr.IndexManager.Collection.classification.CollectionName`).                                                               |
| SOLR_MAIN_CONFIGSET            | mycore_main         | The Solr configset template for the main collection. Sets `MCR.Solr.IndexManager.Collection.main.ConfigSetTemplate`. Only used in cloud mode.                                                                                                                                                     |
| SOLR_CLASSIFICATION_CONFIGSET  | mycore_classification | The Solr configset template for the classification collection. Sets `MCR.Solr.IndexManager.Collection.classification.ConfigSetTemplate`. Only used in cloud mode.                                                                                                                                 |
...
| DEVELOPER_MODE                 | none                | If `true`,  confguration properties to enable the [developer mode](https://www.mycore.de/documentation/developer/dev_devmode/) will be added, the folder configured by `MIR_OVERRIDE` will be mounted as a override folder and the developer commands will be enabled.                            |
