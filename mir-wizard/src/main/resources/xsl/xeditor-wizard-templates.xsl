...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xed="http://www.mycore.de/xeditor" xmlns:i18n="xalan://org.mycore.services.i18n.MCRTranslation"
  xmlns:wizard="http://www.mycore.de/mir/wizard" exclude-result-prefixes="xsl i18n wizard"
>

  <xsl:include href="copynodes.xsl" />

  <xsl:param name="MIR.Wizard.Layout.inputSize" />
  <xsl:param name="MIR.Wizard.Layout.inputWidth" />

  <xsl:variable name="grid-width" select="12" />
  <xsl:variable name="input-size">
    <xsl:choose>
      <xsl:when test="string-length($MIR.Wizard.Layout.inputSize) &gt; 0">
        <xsl:value-of select="$MIR.Wizard.Layout.inputSize" />
      </xsl:when>
      <xsl:otherwise>
  ...
  <xsl:variable name="input-width">
    <xsl:choose>
      <xsl:when test="string-length($MIR.Wizard.Layout.inputWidth) &gt; 0">
        <xsl:value-of select="$MIR.Wizard.Layout.inputWidth" />
      </xsl:when>
      <xsl:otherwise>
  ...
  <xsl:template match="wizard:template[contains('textInput|passwordInput|selectInput|checkboxList|radioList|textArea', @name)]">
  ...
  <xsl:template match="wizard:template[@name='submitButton']">
  ...
  <xsl:template match="wizard:template[@name='cancelButton']">
  ...
  <xsl:template match="wizard:template[contains('textInput|passwordInput|selectInput|checkboxList|radioList|textArea', @name)]" mode="formline">
  ...
  <xsl:template match="wizard:template[contains('textInput|passwordInput|selectInput', @name)]" mode="inline">
  ...
  <xsl:template match="wizard:template[contains('textInput|passwordInput|selectInput|checkboxList|radioList|textArea', @name)]" mode="validation">
  ...
  <xsl:template match="wizard:template[@name='textInput']" mode="widget">
    <input type="text" class="form-control input-{$input-size} {@class}" id="{@id}">
      <xsl:apply-templates select="." mode="inputOptions" />
    </input>
  </xsl:template>

  <xsl:template match="wizard:template[@name='passwordInput']" mode="widget">
    <input type="password" class="form-control input-{$input-size} {@class}" id="{@id}">
      <xsl:apply-templates select="." mode="inputOptions" />
    </input>
  </xsl:template>

  <xsl:template match="wizard:template[@name='selectInput']" mode="widget">
  ...
  <xsl:template match="wizard:template[@name='textArea']" mode="widget">
  ...
  <xsl:template match="wizard:template[@name='checkboxList' or @name='radioList']" mode="widget">
  ...
  <xsl:template match="wizard:template" mode="inputOptions">
  ...
  <xsl:template match="wizard:template" mode="label">
    <label for="{@id}" class="col-md-{$label-width} control-label">
      <xed:output i18n="{@i18n}" />
    </label>
  </xsl:template>

  <xsl:template match="wizard:template" mode="inputTooltip">
    <xsl:if test="@tooltip">
