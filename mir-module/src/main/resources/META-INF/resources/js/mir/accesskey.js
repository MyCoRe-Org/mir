...
class MIRAccessKeyEditor {
    ...
    _fetchI18n() {
        let result = "";
        $.ajax({
            url: webApplicationBaseURL + "rsc/locale/translate/" + $("html").attr("lang") + "/mir.accesskey*",
            type: 'GET',
    ...
    _getI18n(code) {
        ...
        if (this._i18n !== undefined) {
            ...
            return this._i18n["mir.accesskey." + code];
        } else {
            return undefined;
        }
    }
    ...
    _initModalHandler() {
        ...
        const handleDeleteButtonClicked = () => {
            ...
            this._client.deleteKey(value, (error, data) => {
                if (!error) {
                    const index = this._tableModel.data
                        .findIndex(key => key.value === value);
                    if (index == -1) {
        ...
        const handleUpdateButtonClicked = () => {
            ...
            this._client.updateKey(accessKeyId, accessKey, (error, data) => {
                if (!error) {
                    const index = this._tableModel.data
                        .findIndex(key => key.value === accessKeyId);
                    if (index == -1) {
                        console.error("inconsistency");
                    } else {
    ...
    _renderPagination() {
        ...
        if (pageCount > 0 && pageCount > items.length - 2) {
            ...
            if (pageOffset < middle - 1) {
                ...
                items.eq(items.length - (pageCount - pageOffset + 1))
                    .addClass("active");
            } else {
                items.eq(2).find("a").text("..");
                items.eq(2).addClass("disabled");
                items.eq(items.length - 3).find("a").text("..");
                items.eq(items.length - 3).addClass("disabled");
                let current = Math.floor((pageOffset + 1) 
                    - ((items.length - 6 - 1) / 2));
