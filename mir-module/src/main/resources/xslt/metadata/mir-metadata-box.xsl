                ...
                <td class="metavalue">
                  <xsl:for-each select="key('title-by-type',@type)">
                    ...
                    <xsl:if test="@type='translated'">
                      <xsl:text> (</xsl:text>
                      <xsl:value-of select="mcrclassification:current-label-text(mcrclassification:category('rfc5646',@xml:lang))" />
                      <xsl:text>)</xsl:text>
                    </xsl:if>
                  </xsl:for-each>
                </td>
  ...
  <xsl:template mode="display-metadata" match="mods:physicalDescription">
    ...
    <xsl:if test="$form">
      ...
      <xsl:call-template name="printMetaDate.mods">
        ...
        <xsl:with-param name="label" select="mcri18n:translate('mir.physical.description.form')" />
      </xsl:call-template>
    </xsl:if>
    ...
    <xsl:if test="$note">
      <xsl:call-template name="printMetaDate.mods">
        ...
        <xsl:with-param name="label" select="mcri18n:translate('mir.physical.description.note')" />
      </xsl:call-template>
    </xsl:if>
    <xsl:call-template name="printMetaDate.mods">
