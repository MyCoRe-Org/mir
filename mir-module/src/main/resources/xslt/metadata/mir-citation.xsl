...
<xsl:stylesheet version="3.0"
  ...
  <xsl:include href="resource:xslt/mods-dc-meta.xsl"/>
  <xsl:include href="resource:xslt/mods-seo-meta.xsl"/>
  <xsl:include href="resource:xslt/mods-highwire.xsl" />
  ...
  <xsl:template match="/">
    ...
    <xsl:variable name="piServiceInformation" select="mcrpi:get-pi-service-information(mycoreobject/@ID)" />
    ...
    <div id="mir-citation">
      ...
      <div id="citation-style">
        ...
        <xsl:if test="string-length($MIR.citationStyles) &gt; 0">
          ...
          <select aria-label="select Citation style" class="form-control input-sm form-select" id="mir-csl-cite"
  ...
  <xsl:template match="mods:mods" mode="year">
    ...
    <xsl:if test="string-length($dateIssued) &gt; 0">
      <xsl:value-of select="format-date($dateIssued, '[Y0001]')"/>
    </xsl:if>
  </xsl:template>
  ...
  <xsl:template name="identifierEntry">
    ...
    <xsl:if test="string-length($id) &gt; 0">
      <div class="mir_identifier">
        ...
        <input aria-label="Identifier to copy" type="text" class="d-none mir_identifier_hidden_input" value="{$id}"></input>
      </div>
    </xsl:if>
  </xsl:template>
