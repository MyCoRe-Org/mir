...
<xsl:stylesheet version="3.0"
  ...
  <xsl:variable name="revision" select="mcrurl:get-param($RequestURL,'r')" />
  ...
  <xsl:template match="/">
    ...
    <div id="mir-abstract-plus">
      ...
      <xsl:if test="$mods/mods:abstract">
        ...
        <xsl:variable name="abstracts" select="$aFilter" />
        ...
        <xsl:choose>
          <xsl:when test="count($abstracts/mods:abstract) &gt; 1">
            ...
            <xsl:variable name="first-abstract-in-current-lang-position-string"
                          select="normalize-space($first-abstract-in-current-lang-position)"/>
            <xsl:variable name="has-abstract-in-current-lang"
                          select="string-length($first-abstract-in-current-lang-position-string) &gt; 0"/>
                ...
                <xsl:for-each select="$abstracts/mods:abstract">
                  ...
                  <xsl:variable name="tabName">
                    <xsl:choose>
                      <xsl:when test="@type and $MIR.Layout.Abstract.Type.Classification">
                        <xsl:value-of select="mcrclassification:current-label-text(mcrclassification:category($MIR.Layout.Abstract.Type.Classification, @type))"/>
                      </xsl:when>
                  ...
                  <li class="nav-item">
                    <a class="nav-link" href="#tab{position()}" role="tab" data-bs-toggle="tab">
                      <xsl:choose>
                        <xsl:when test="$has-abstract-in-current-lang and $first-abstract-in-current-lang-position-string = string(position())">
                        <xsl:attribute name="class">active nav-link</xsl:attribute>
                        </xsl:when>
                        <xsl:otherwise>
                          <xsl:if test="position() = 1 and not($has-abstract-in-current-lang)">
                            <xsl:attribute name="class">active nav-link</xsl:attribute>
                          </xsl:if>
                        </xsl:otherwise>
                      </xsl:choose>
  ...
  <xsl:template name="findRelatedItems">
    ...
    <xsl:variable name="hitsSortList"
                  select="document(concat('solr:q=',encode-for-uri($query), '&amp;rows=1000&amp;sort=mods.part.order.', $objectID, '%20desc,mods.dateIssued%20desc,%20mods.dateIssued.host%20desc,',  $modsPart, '%20desc,%20mods.title.main%20desc&amp;group=true&amp;group.limit=1000&amp;group.field=mods.yearIssued'))/response/lst[@name='grouped']/lst[@name='mods.yearIssued']" />
