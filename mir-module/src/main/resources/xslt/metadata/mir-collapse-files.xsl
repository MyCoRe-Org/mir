          ...
          <xsl:choose>
            <xsl:when test="mycoreobject/structure/derobjects/derobject and not(mycoreobject/structure/derobjects/derobject[key('rights', @xlink:href)/@read])">
              <div id="mir-access-restricted">
                ...
                <div class="alert alert-warning" role="alert">
                  <xsl:variable name="embargoDate" select="document(concat('callJava:org.mycore.mods.MCRMODSEmbargoUtils:getEmbargo:',mycoreobject/@ID))" />
                  <xsl:choose>
                    <xsl:when test="string-length($embargoDate)&gt;0">
                      <!-- embargo is active for guest user -->
                      <xsl:value-of select="mcri18n:translate-with-params('component.mods.metaData.dictionary.accessCondition.embargo.available',$embargoDate)" />
                    </xsl:when>
                    <xsl:when test="mycoreobject/metadata/def.modsContainer/modsContainer/mods:mods/mods:accessCondition[@type='restriction on access'][substring-after(@xlink:href,'#')='intern']">
                      <xsl:value-of disable-output-escaping="yes" select="mcri18n:translate-with-params('mir.derivate.no_access.intern',$loginURL)" />
                    </xsl:when>
            ...
            <xsl:otherwise>
              ...
              <xsl:for-each select="mycoreobject/structure/derobjects/derobject[key('rights', @xlink:href)/@read]">
                ...
                <xsl:variable name="derivateDisplayed" select="mcrderivate:is-display-enabled(@xlink:href, 'show-file-area')" />
                <xsl:if test="$derivateDisplayed or key('rights', $derId)/@write">
                  <div id="files{@xlink:href}" class="file_box">
                              ...
                              <span>
                                <xsl:choose>
                                  ...
                                  <xsl:when test="string-length($derivateType)!=0">
                                    <xsl:value-of select="mcrclassification:current-label-text(mcrclassification:category('derivate_types',$derivateType))" />
                                  </xsl:when>
                                  <xsl:otherwise>
                                    <xsl:value-of select="mcri18n:translate('metadata.files.file')" />
                                  </xsl:otherwise>
                    ...
                    <xsl:choose>
                      <xsl:when test="key('rights', @xlink:href)/@read">
                        ...
                        <div class="file_box_files" data-objID="{$objID}" data-deriID="{$derId}" data-mainDoc="{$maindoc}" data-writedb="{mcracl:check-permission($derId,'writedb')}" data-deletedb="{mcracl:check-permission($derId,'deletedb')}" data-numperpage="{$MIR.FileBrowser.FilesPerPage}">
                          <xsl:if test="mcracl:check-permission($derId,'read')">
