...
-->
  ...
  <xsl:template match="mods:classification" mode="dc">
    ...
    <xsl:variable name="mycoreclass" select="mcrmods:to-mycoreclass(., 'single')" />
  ...
  <xsl:template match="mods:accessCondition[@type='use and reproduction']" mode="dc">
    ...
    <dc:rights>
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:value-of select="mcrclassification:current-label-text(mcrclassification:category('mir_licenses',$trimmed))" />
        </xsl:otherwise>
      </xsl:choose>
    </dc:rights>
