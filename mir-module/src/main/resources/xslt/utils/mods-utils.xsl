...
<xsl:stylesheet version="3.0"
    ...
    <xsl:choose>
      ...
      <xsl:when test="mods:titleInfo/mods:title">
        <xsl:choose>
          ...
          <xsl:when test="string-length($type) &gt; 0 and string-length($position) &gt; 0">
            <xsl:apply-templates select="mods:titleInfo[@type=$type][string(position())=$position]" mode="mods.printTitle">
              <xsl:with-param name="asHTML" select="$asHTML" />
              <xsl:with-param name="withSubtitle" select="$withSubtitle" />
            </xsl:apply-templates>
          </xsl:when>
  ...
  <xsl:template match="mods:name" mode="nameLink">
    ...
    <a href="concat($ServletsBaseURL, 'solr/mods_nameIdentifier?q=', encode-for-uri($query), '&amp;owner=createdby:', $owner)">
      <span>
        <span>
