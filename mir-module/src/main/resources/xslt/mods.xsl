...
<!-- URN -->
  ...
  <xsl:template priority="1" mode="present" match="/mycoreobject[contains(@ID,'_mods_')]">
    ...
    <div id="detail_view" class="blockbox" typeOf="{$rdfaTypeOf}">
      ...
      <xsl:variable name="derivateDisplayList">
        ...
        <xsl:for-each select="./structure/derobjects/derobject">
          <derivate>
            ...
            <display><xsl:value-of select="mcracl:checkDerivateDisplayPermission(@xlink:href)" /></display>
          </derivate>
        </xsl:for-each>
        </derivates>
      </xsl:variable>
      ...
      <xsl:if
        test="((./structure/children/child) and not($mods-type='series' or $mods-type='journal' or $mods-type='confpro' or $mods-type='book')) or (./structure/derobjects/derobject and ($isDisplayedEnabled or not(mcracl:is-current-user-in-role('guest'))))">
        <div id="derivate_box" class="detailbox">
          <h4 id="derivate_switch" class="block_switch">
            <a name="derivate_box"></a>
            <xsl:value-of select="mcri18n:translate('component.mods.metaData.dictionary.derivatebox')" />
          </h4>
                ...
                <xsl:variable name="label">
                  <xsl:choose>
                    <xsl:when test="count($context/structure/children/child[contains(@xlink:href,$thisObjectType)])=1">
                      <xsl:value-of select="mcri18n:translate(concat('component.',$thisObjectType,'.metaData.[singular]'))" />
                    </xsl:when>
                    <xsl:otherwise>
                      <xsl:value-of select="mcri18n:translate(concat('component.',$thisObjectType,'.metaData.[plural]'))" />
                    </xsl:otherwise>
                  </xsl:choose>
                </xsl:variable>
        ...
        <div id="system_box" class="detailbox">
          ...
          <div id="system_content" class="block_content">
            <table class="metaData">
              ...
              <tr>
                <td class="metaname">
                  <xsl:value-of select="concat(mcri18n:translate('metaData.versions'),' :')" />
                </td>
                <td class="metavalue">
    ...
    <xsl:if test="./structure/derobjects">
      ...
      <tr>
        <td style="vertical-align:top;" class="metaname">
          <xsl:value-of select="concat(mcri18n:translate('component.mods.metaData.[derivates]'), ':')" />
        </td>
        <td class="metavalue">
          <xsl:for-each select="./structure/derobjects/derobject">
...
<!--  TODO:             <xsl:variable name="derivateWithURN" select="mcrurn:hasURNDefined($deriv)" />-->
              <xsl:variable name="derivateWithURN" select="false()" />
              ...
              <xsl:if test="mcracl:check-permission(./@xlink:href,'writedb')">
                <div class="derivate_options">
                  <!-- TODO: refacture! -->
                  <img class="button_options" src="{$WebApplicationBaseURL}images/icon_arrow_circled_red_down.png"
                       alt="show derivate options" title="{mcri18n:translate('component.mods.metaData.options')}" />
                  <div class="options">
                    <ul>
                      <xsl:if test="$derivateWithURN=false()">
                        <li>
                          <a href="{$ServletsBaseURL}derivate/update?objectid={../../../@ID}&amp;id={@xlink:href}{$suffix}">
                            <xsl:value-of select="mcri18n:translate('component.mods.derivate.addFile')" />
                          </a>
                        </li>
                      </xsl:if>
                      <li>
<!--     TODO:                   <xsl:if test="not($derivateWithURN=false() and mcrxsl:isAllowedObjectForURNAssignment($parentObjID))">-->
                        <xsl:if test="not($derivateWithURN=false() and false())">
                          <xsl:attribute name="class">last</xsl:attribute>
                        </xsl:if>
                        <a href="{$ServletsBaseURL}derivate/update?id={@xlink:href}{$suffix}">
                          <xsl:value-of select="mcri18n:translate('component.mods.derivate.editDerivate')" />
                        </a>
                      </li>
<!--                      <xsl:if test="$derivateWithURN=false() and mcrxsl:isAllowedObjectForURNAssignment($parentObjID)">-->
                      <xsl:if test="$derivateWithURN=false() and false()">
                        <xsl:variable name="apos">
                          <xsl:text>'</xsl:text>
                        </xsl:variable>
                        <li>
                          <xsl:if test="not(mcracl:check-permission(./@xlink:href,'deletedb'))">
                            <xsl:attribute name="class">last</xsl:attribute>
                          </xsl:if>
                          <a href="{$ServletsBaseURL}MCRAddURNToObjectServlet?object={@xlink:href}" onclick="{concat('return confirm(',$apos, mcri18n:translate('component.mods.metaData.options.urn.confirm'), $apos, ');')}">
                            <xsl:value-of select="mcri18n:translate('component.mods.metaData.options.urn')" />
                          </a>
                        </li>
                      </xsl:if>
                      <xsl:if test="mcracl:check-permission(./@xlink:href,'deletedb') and $derivateWithURN=false()">
                        <li class="last">
                          <a href="{$ServletsBaseURL}derivate/delete?id={@xlink:href}" class="confirm_derivate_deletion">
                            <xsl:value-of select="mcri18n:translate('component.mods.derivate.delDerivate')" />
                          </a>
                        </li>
                      </xsl:if>
                    </ul>
  ...
  <xsl:template match="/mycoreobject[contains(@ID,'_mods_')]" mode="objectActions" priority="1">
    ...
    <xsl:param name="accesscreate" select="mcracl:check-permission('create-mods')" />
    <xsl:param name="hasURN" select="'false'" />
    <xsl:param name="displayAddDerivate" select="mcracl:check-permission($id,'writedb')" />
    ...
    <xsl:variable name="hasDerivateWithURN">
      <xsl:for-each select="./structure/derobjects/derobject">
<!--    TODO:    <xsl:variable name="hasUrlDefined" select="mcrurn:hasURNDefined(@xlink:href)"/>-->
        <xsl:variable name="hasUrlDefined" select="false()"/>
        <xsl:if test="$hasUrlDefined">
    ...
    <xsl:choose>
      <xsl:when test="$accessedit or $accessdelete or $accesscreate">
        ...
        <div class="document_options">
          <!-- TODO: refacture! -->
          <img class="button_options" src="{$WebApplicationBaseURL}images/icon_arrow_circled_red_down.png"
            alt="show document options" title="{mcri18n:translate('component.mods.metaData.options')}" />
          <div class="options">
            <ul>
              <xsl:if test="$accessedit or $accesscreate">
                ...
                <xsl:if test="$displayAddDerivate='true'">
                  <li>
                    <xsl:if test="not($CurrentUser=$MCR.Users.Superuser.UserName) and
                                  $accessdelete and $hasDerivateWithURN and
                                  string-length($child-layout)=0 and not(mcracl:check-permission(./@ID,'writedb'))">
                      <xsl:attribute name="class">last</xsl:attribute>
                    </xsl:if>
              ...
              <xsl:if test="$accessdelete and string-length($hasDerivateWithURN)=0">
                <li>
                  <xsl:if test="not($CurrentUser=$MCR.Users.Superuser.UserName) and string-length($child-layout)=0 and not(mcracl:check-permission(./@ID,'writedb'))">
                    <xsl:attribute name="class">last</xsl:attribute>
                  </xsl:if>
                  <xsl:choose>
                    <xsl:when test="/mycoreobject/structure/children/child">
                      <xsl:value-of select="mcri18n:translate('object.hasChildren')" />
                    </xsl:when>
                    <xsl:otherwise>
                  ...
                  <xsl:when test="$mods-type = 'series'">
                    <li>
                      <a href="{$ServletsBaseURL}object/create?type=mods&amp;layout=book&amp;parentId={./@ID}">
                        <xsl:value-of select="mcri18n:translate('component.mods.metaData.types.book')" />
                      </a>
                    </li>
                    <li>
                      ...
                      <a href="{$ServletsBaseURL}object/create?type=mods&amp;layout=confpro&amp;parentId={./@ID}">
                        <xsl:value-of select="mcri18n:translate('component.mods.metaData.types.confpro')" />
                      </a>
                    </li>
                  </xsl:when>
                  <xsl:otherwise>
                    <li>
                      ...
                      <a href="{$ServletsBaseURL}object/create?type=mods&amp;layout={$child-layout}&amp;parentId={./@ID}">
                        <xsl:value-of select="mcri18n:translate(concat('component.mods.metaData.types.',$child-layout))" />
                      </a>
                    </li>
                  </xsl:otherwise>
                </xsl:choose>
