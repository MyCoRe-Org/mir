...
<xsl:stylesheet version="3.0"
  ...
  <xsl:template match="mycoreobject[contains(@ID,'_mods_')]">
    <xsl:variable name="statePublishedCateg" select="document('classification:metadata:-1:children:state:published')" />
    <xsl:variable name="objState" select="service/servstates[@class='MCRMetaClassification']/servstate[@classid='state']/@categid" />
    <xsl:variable name="status" select="count($statePublishedCateg//category[@ID=$objState]) &gt; 0" />
    <xsl:apply-imports />
