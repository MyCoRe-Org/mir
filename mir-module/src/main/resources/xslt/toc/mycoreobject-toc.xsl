...
<xsl:stylesheet version="3.0"
  ...
  <xsl:template match="/">
    ...
    <xsl:variable name="tocLayouts"
      select="document('xslStyle:toc/toc-layouts2solr-json-facet-query:resource:toc-layouts.xml')/*" />
    ...
    <xsl:variable name="q">
      ...
      <xsl:choose>
        <xsl:when test="mcracl:is-current-user-in-role('admin')">*</xsl:when>
        <xsl:when test="mcracl:is-current-user-in-role('editor')">*</xsl:when>
    ...
    <xsl:variable name="solrURI">
      <xsl:text>solr:q=</xsl:text><xsl:value-of select="encode-for-uri($q)" />
    ...
    <xsl:variable name="htmlURI"
      select="concat('notnull:xslStyle:toc/render-toc:', $prepURI)" />
