...
<xsl:stylesheet version="2.0"
  ...
  <xsl:variable name="creatorRoles"
                select="$marcrelator/mycoreclass/categories/category[@ID='cre']/descendant-or-self::category" xmlns=""/>
  <xsl:variable name="authorRoles"
                select="$marcrelator/mycoreclass/categories/category[@ID='aut']/descendant-or-self::category" xmlns=""/>
  ...
  <xsl:template match="mycoreobject" mode="validateChannel">
    ...
    <xsl:sequence select="if (count($mods/mods:abstract) != 1)
  ...
  <xsl:template match="mods:mods" mode="description">
    <xsl:if test="mods:abstract">
      <description xml:space="preserve">
        <xsl:value-of select="fn:zero-or-one(mods:abstract)"/>
      </description>
    </xsl:if>
  </xsl:template>
  ...
  <xsl:template match="mycoreobject" mode="validateItem">
    ...
    <xsl:if test="count($mods/mods:abstract) != 1 and not($mods/mods:relatedItem[@type='host']/mods:part/@order = 0)">
      <xsl:comment>
