...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                ...
                version="1.0" exclude-result-prefixes="i18n exslt" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
                xsi:schemaLocation="xalan://org.mycore.validation.pdfa.MCRPdfAFunctions ">
        ...
        <xsl:if test="normalize-space($MIR.Workflow.Box)='true'">
            <div id="mir-workflow" class="col-sm-12">
                <xsl:variable name="statusClassification"
                              select="document('classification:metadata:-1:children:state')"/>
                <xsl:variable name="currentStatus"
                              select="/mycoreobject/service/servstates/servstate[@classid='state']/@categid"/>
                <xsl:variable name="creator"
                              select="/mycoreobject/service/servflags/servflag[@type='createdby']/text()"/>
            ...
            <xsl:variable name="message">
                <p>
                    ...
                    <ul>
                        ...
                        <xsl:choose>
                            <xsl:when
                                    test="normalize-space($MIR.Workflow.ReviewDerivateRequired) = 'true' and not(structure/derobjects/derobject/maindoc)">
                                <li>
                                    <a href="#">
                                        <xsl:attribute name="onclick">
                                            document.querySelector('[data-upload-object]').scrollIntoView({behavior:
                                            'smooth', block: 'center', inline: 'nearest'}); return false;
                                        </xsl:attribute>
                                        <xsl:value-of
                                                select="i18n:translate('mir.workflow.creator.submitted.require.derivate')"/>
                                    </a>
                                </li>
                            </xsl:when>
                            <xsl:otherwise>
                        ...
                        <xsl:if test="string-length($MIR.Sherpa.API.Key)&gt;0">
                            ...
                            <xsl:if test="string-length($issn)&gt;0">
                                <li data-sherpainfo-issn="{$issn}">
                                    <xsl:value-of select="i18n:translate('mir.workflow.sherpa.loading')"/>
