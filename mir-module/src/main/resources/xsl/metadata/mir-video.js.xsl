...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                ...
                exclude-result-prefixes="xalan i18n mcr media mods xlink FilenameUtils iview2 mcrxsl mcrsolr mcrsolru encoder"
>
  ...
  <xsl:template match="/">
    ...
    <xsl:if test="mcrsolr:getNumFound($solrQuery) &gt; 0">
      ...
      <xsl:variable name="solrResult"
                    select="document($completeQuery)" /> <!-- [string-length(str[@name='groupValue']/text()) &gt; 0] -->
      <div id="mir-player">
