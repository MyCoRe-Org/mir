...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:mods="http://www.loc.gov/mods/v3"
  ...
  exclude-result-prefixes="mcrxsl i18n xlink exslt" xmlns:ex="http://exslt.org/dates-and-times" extension-element-prefixes="ex">
  ...
  <xsl:template match="derobjects">
    ...
    <xsl:variable name="nodes">
      <xsl:for-each select="derobject">
        <xsl:if test="mcrxsl:isDisplayedEnabledDerivate(@xlink:href) or not(mcrxsl:isCurrentUserGuestUser())">
          <xsl:choose>
            <xsl:when
