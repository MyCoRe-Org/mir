...
import { OrcidUserSettingsModalHandler } from './orcid/orcid-user-settings-modal-handler';
import { OrcidApiClientFactory } from './orcid/orcid-utils';
import { getBaseUrl } from './utils/config';
import { getElementByIdOrThrow } from './utils/ui';

const setupModalHandler = (): void => {
  ...
  const orcidUserSettingsModalHandler = new OrcidUserSettingsModalHandler({
    modalEl,
  ...
  document.body.addEventListener(
    ...
    async (e: MouseEvent): Promise<void> => {
      const btn = (e.target as Element).closest('.open-orcid-settings');
      if (!(btn instanceof HTMLButtonElement)) return;
      if (btn.dataset.orcid)
        await orcidUserSettingsModalHandler!.open(btn.dataset.orcid);
    }
  );
};
...
const setupAuthRevocationHandler = (): void => {
  document.body.addEventListener(
    'click',
...
const setupAuthInitHandler = (): void => {
  document
...
document.addEventListener('DOMContentLoaded', (): void => {
  setupAuthInitHandler();
  setupAuthRevocationHandler();
  setupModalHandler();
});
