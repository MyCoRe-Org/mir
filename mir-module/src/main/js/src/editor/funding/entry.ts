  ...
  try {
    ...
    new Autocomplete<Funder>(funderNameInput, {
      fetchData: (query, signal) =>
        fetchFunder(query, signal).then(items =>
          items.map(r => ({
            name: r.name,
            id: { type: 'Crossref Funder ID', value: r.uri },
          }))
        ),
...
export function initFundingAutocomplete(): void {
  document
    .querySelectorAll('.mir-funding-reference-container')
    .forEach(setupFundingAutocomplete);
}

document.addEventListener('DOMContentLoaded', initFundingAutocomplete);
