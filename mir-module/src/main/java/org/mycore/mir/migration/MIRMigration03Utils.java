...
public class MIRMigration03Utils {
    ...
    public static List<String> updateStateClassification() {
        ...
        //get all categories:
        MCRCategory mirStatus = MCRCategoryDAOFactory.getInstance().getCategory(new MCRCategoryID("mir_status"), -1);
        if (mirStatus == null) {
            LOGGER.info("No classification 'mir_status' found.");
            return null;
        }
        boolean stateClassPresent = MCRCategoryDAOFactory.getInstance().exist(new MCRCategoryID("state"));
