...
public class MIRMigration201603Utils {
    ...
    public static List<String> updateStateClassification() {
        ...
        //get all categories:
        MCRCategory mirRights = MCRCategoryDAOFactory.getInstance().getCategory(new MCRCategoryID("mir_rights"), -1);
        if (mirRights == null) {
        ...
        TreeSet<String> ids = new TreeSet<>(MCRXMLMetadataManager.getInstance().listIDsOfType("mods"));
        ArrayList<String> cmds = new ArrayList<>(ids.size());
        MCRCategory mirLicenses = MCRCategoryDAOFactory.getInstance().getCategory(new MCRCategoryID("mir_licenses"),
            -1);
        if (mirLicenses == null) {
